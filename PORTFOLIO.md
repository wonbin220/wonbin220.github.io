...
- **Backend**:
    ...
    - **Spring Boot 3.4.5** : 자동 구성(Auto-configuration)과 내장 서버를 통해 초기 설정의 복잡함을 줄여 핵심 비즈니스 로직 개발에 집중할 수 있음. 스타터(starter) 의존성은 라이브러리 버전 관리를 간소화하여 안정성을 높여주었고, Spring Security 등 다른 Spring 프로젝트와의 뛰어난 통합 덕분에 보안과 같은 필수 기능들을 효율적으로 구현할 수 있었음.
